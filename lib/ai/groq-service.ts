...
export async function enhanceBulletPoint(
  ...
  if (!isLLMConfigured()) {
    console.warn("No LLM API key set. Returning original bullet point.")
    return bulletPoint
  }
...
Rewrite this experience bullet point following these strict rules:
...
6. Make it achievement-focused, not task-focused`
  ...
  try {
    return await retryWithBackoff(async () => {
      return await llmGenerate(prompt, { temperature: 0.3, maxTokens: 256, fast: true }, systemPrompt)
    })
  } catch (error) {
...
5. Preserve the original meaning.`
  ...
  try {
    const enhancedBullets = await retryWithBackoff(async () => {
      const raw = await llmGenerate(prompt, { temperature: 0.3, maxTokens: 2048, jsonMode: true, fast: true }, systemPrompt)
      const parsed = parseModelJson<{ bullets?: string[] }>(raw)
...
export async function extractSkills(resumeText: string): Promise<SkillsCategories> {
  if (!isLLMConfigured()) {
    console.warn("No LLM API key set. Returning empty skills.")
...
Output format (JSON only):
{
  ...
  try {
    const result = await retryWithBackoff(async () => {
      const raw = await llmGenerate(prompt, { temperature: 0.2, maxTokens: 1024, jsonMode: true })
      const parsed = parseModelJson<Partial<SkillsCategories>>(raw)
      if (!parsed) {
    ...
    console.warn("Could not extract skills. Falling back to empty categories.", error)
    return { languages: [], frameworks: [], tools: [], databases: [] }
  }
}
...
export async function enhanceSection(
  ...
  const prompt = `Enhance this ${sectionType} section for ATS optimization.
...
Return ONLY the enhanced content, one item per line.`
  ...
  try {
    const result = await retryWithBackoff(async () => {
      const raw = await llmGenerate(prompt, { temperature: 0.3, maxTokens: 2048, fast: true })
...
Return ONLY the professional summary, no additional text.`
  ...
  try {
    return await retryWithBackoff(async () => {
      return await llmGenerate(prompt, { temperature: 0.3, maxTokens: 512, fast: true })
    })
  } catch (error) {
    console.warn("Could not generate summary. Using default.", error)
    return "Experienced professional with a proven track record of success."
  }
}
...
export async function extractCompleteResumeData(resumeText: string): Promise<any> {
  ...
  const systemPrompt = `You are an expert resume parser. Extract EVERY piece of information from the resume into structured JSON.

ZERO DATA LOSS RULE: Do NOT skip ANY content. If you see a section you don't recognize, add it to "customSections".`

  const prompt = `EXTRACTION REQUIREMENTS:
...
10. VOLUNTEER WORK (extract if present):
    - Organization, role, location, dates, bullet points
...
export async function enhanceExtractedData(extractedData: any): Promise<any> {
  ...
  try {
    ...
    console.warn("Could not apply AI enhancement. Using extracted data as-is.", error)
    return extractedData
  }
}
...
export { isLLMConfigured as isAIConfigured }
